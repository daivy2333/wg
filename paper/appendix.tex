...
本文实验共涉及10个模型，涵盖传统机器学习与深度学习方法。表6列出6组核心配置的关键超参数，其余4个模型（$K$近邻、朴素贝叶斯、逻辑回归、支持向量机）采用scikit-learn库默认参数，不再单独列出。所有神经网络模型的优化器统一为Adam，批次大小为256，训练轮数上限为50，并启用早停机制（patience=5）。
...
\toprule
模型 & Accuracy & DoS F1 & Normal F1 & Probe F1 & R2L F1 \\
\midrule
DT  & 0.743 & 0.89 & 0.82 & 0.74 & 0.26 \\
RF  & 0.737 & 0.88 & 0.81 & 0.73 & 0.23 \\
MLP & 0.810 & 0.88 & 0.82 & 0.75 & 0.57 \\
\bottomrule
\end{tabular}
\end{table}

R2L和U2R类别因训练样本极度稀缺（分别仅995条和52条），F1值显著低于DoS、Normal和Probe。MLP在R2L上的F1（0.57）明显优于树模型（0.23-0.26），展现了神经网络在特征交互学习上的优势。然而，U2R类（仅52条训练样本）对所有模型而言仍然是极大的挑战，F1值均未超过0.6。
