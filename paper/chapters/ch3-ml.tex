...
两类模型均采用二分类 (Normal vs Anomaly) 与多分类 (40 类攻击标签) 两种任务设置，
...
将上述最优决策树直接应用于 40 类攻击标签的多分类任务，
测试集准确率仅为 acc=0.0456、macro-F1=0.0155。
该数值约为 40 类随机猜测基线 $1/40 \approx 0.025$ 的 1.8 倍，
模型几乎完全失效。
...
DT 与 RF 的多分类准确率均仅约 4.6\%，仅略高于 40 类随机猜测基线（2.5\%）。
