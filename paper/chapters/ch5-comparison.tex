...
图13 以并列柱状图形式直观呈现传统机器学习与深度学习方法
在 accuracy、precision、recall、F1、AUC 五个维度上的差异，可视化地印证了上述分析。

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{13_dl_vs_ml_comparison.png}
...
图14 将五个主要模型的 ROC 曲线叠加在同一坐标系中，
曲线下面积（AUC）直接反映了模型的全局判别能力。

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{11_roc_curves.png}
...
为深入理解多分类任务中各模型的细粒度表现，图15 给出了
DT、RF、MLP 三个代表性模型在 Normal 与四类攻击大类（DoS、Probe、R2L、U2R）上的 F1 柱状图。

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{10_f1_by_category.png}
...
图16 并列展示两棵树模型的前 15 维特征重要度。

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{12_feature_importance_comparison.png}
