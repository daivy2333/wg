...
在 IID 分布的验证集上，MLP 与 DT/RF 的二分类 F1 均接近 0.99，验证集指标无法区分模型优劣；
但在包含 15 种 unseen 攻击的 OOD 测试集上，MLP 的 F1 跌至 0.7202，DT 为 0.7621，RF 为 0.6882。
深度学习方法在分布外测试集上的泛化衰减幅度（MLP 下降 27pp）显著大于传统方法（DT 下降 23pp），
说明深度模型的过拟合风险更高。

这一现象的根源有三：第一，NSL-KDD 每条样本代表一次独立的网络连接，
样本之间无时序或空间局部结构，MLP/CNN/LSTM 的归纳偏置（局部性、平移不变性、时序依赖）均无法发挥作用；
第二，训练集与测试集攻击分布差异巨大，模型在训练阶段从未见过 15 种新型攻击的任何样本，
泛化只能依赖特征空间的相似性度量，而神经网络对未见样本的相似性度量并无系统性优势；
第三，DT/RF 的硬决策与高 interpretability 让其决策边界更易于审计与人工修正，
在安全敏感场景中具有工程层面的隐性优势。

表5 进一步比较了三模型的多分类表现。
MLP 的准确率达到 81.0\%，DT 与 RF 分别为 74.3\% 和 73.7\%，
MLP 在五项指标上全面领先，展现了神经网络在特征交互学习上的优势。
然而三者的 R2L 和 U2R 类 F1 仍然偏低（R2L 0.23-0.58，U2R 0.06-0.56），再次印证小类的系统性失败主导了整体指标。
...
其二，所有模型在验证集到测试集上均存在性能衰减，但程度因模型而异。
DT 的验证-测试差距最小（CV-F1 约 0.77→0.76，仅 1pp），RF 和 MLP 的衰减幅度较大（分别约 30pp 和 27pp）。
这说明复杂模型更容易在 IID 验证集上过拟合，简单的树模型反而对分布偏移更加鲁棒。
