...
Probe 类处于 0.7-0.75 的水平，R2L 接近 0.2-0.6，而 U2R 仍普遍偏低（MLP 约 56\%，DT/RF 显著更低）。
这一格局与训练集样本分布高度相关：U2R 在训练集中仅有 52 条样本，
分类器几乎无法从中学习到稳定的判别模式，遑论对测试集中同类变种进行泛化。
即便 MLP 在 Normal 和 DoS 上的 F1 高于 DT/RF，小类识别仍是所有模型的共同短板。
...
然而三者的 R2L 和 U2R 类 F1 仍然偏低（R2L 0.23-0.58，U2R < 0.57），再次印证小类的系统性失败主导了整体指标。
