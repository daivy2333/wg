...
多分类任务则采用 accuracy 与 f1\_macro（宏平均 F1，对类别不均衡敏感）两项指标，并额外报告各类别 per-class F1 以分析小类识别能力。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    \toprule
    模型 & Accuracy & F1 Macro & R2L F1 \\
    \midrule
    MLP & \textbf{0.810} & \textbf{0.718} & \textbf{0.575} \\
    DT  & 0.743 & 0.488 & 0.256 \\
    RF  & 0.737 & 0.476 & 0.231 \\
    \bottomrule
  \end{tabular}
\end{table}
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{09_confusion_matrix_dt_multiclass.png}
  \caption{决策树五分类混淆矩阵}
  \label{fig:cm-dt-multi}
\end{figure*}
