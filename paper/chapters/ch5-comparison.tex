...
多分类任务则采用 accuracy 与 f1\_macro（宏平均 F1，对类别不均衡敏感）两项指标，各类别 per-class F1 详见附录表6。
...
为深入理解各二分类检测器在不同攻击大类上的区分能力，图14 给出了
DT、RF、MLP 三个代表性二分类模型在 Normal 与四类攻击大类（DoS、Probe、R2L、U2R）上的 F1 柱状图。
...
深度学习方法在分布外测试集上的泛化衰减幅度（MLP 下降 27pp、RF 下降 30pp）显著大于传统方法（DT 仅下降 1pp），
说明深度模型的过拟合风险更高。
